...
do
	...
	if [ "$prevline" != "$line" ]; then
		...
		filename="${filename_date_part}_${freq}@t.mp3"
		...
		test $ti -gt 2 && mp3splt $mp3spltopts $record_file $sm.$ss.$sh $em.$es.$eh -o "${dir1}/${filename}" 2>/dev/null
		test $ti -gt 2 && echo "Splitting ${filename_date_part}_${freq}.mp3 from $record_file to ${dir1}, start $sm.$ss.$sh, end $em.$es.$eh.">> /tmp/split.log
	fi
	prevline="$line"
	sleep 1s
done
